...
module.exports = {
	register : async prm => {
		...
		let rslt = await bc.invoke('soila_chain', 'person.register', 
	...
	get : async prm => {
		...
		rslt = await bc.invoke('soila_chain', 'person.get', [prm.key]);
		if (rslt.status!=200) {
		...
		rslt = await bc.invoke('soila_chain', 'peer.get', [rslt.data[1].peerkey]);
		if (rslt.status!=200) {
	...
	update : async prm => {
		...
		rslt = await bc.invoke('soila_chain', 'person.update', 
